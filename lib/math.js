...
function getBy(arr, fn, op, resultOp) {
    var first = arr[0];
    if (arr.length <= 1) {
        return first;
    }
    var fnString = lang_1.isString(fn);
    return arr.slice(1).reduce(function (current, item) {
        var thisItem = fnString ? object_1.get(current, fn) : fn(current);
        var nextItem = fnString ? object_1.get(item, fn) : fn(item);
        var result = op(thisItem, nextItem);
        return resultOp ? resultOp(result, current, item, nextItem) : result;
    }, first);
}
function compareBy(arr, fn, op) {
    return getBy(arr, fn, op, function (result, current, next, nextValue) {
