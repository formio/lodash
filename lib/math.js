...
function compareBy(arr, fn, op) {
    ...
    return arr.slice(1).reduce(function (current, next) {
        var currentValue = fnString ? object_1.get(current, fn) : fn(current);
        var nextValue = fnString ? object_1.get(next, fn) : fn(next);
...
function valueBy(arr, fn, op) {
    ...
    var fnString = lang_1.isString(fn);
    return arr.slice(1).reduce(function (current, next) { return op(current, fnString ? object_1.get(next, fn) : fn(next)); }, fnString ? object_1.get(first, fn) : fn(first));
}
/**
